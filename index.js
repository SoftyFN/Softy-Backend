...
(function () {
	...
	if (URL_LOGGING)
		app.use((req, res, next) => {
			if (!req.url.endsWith('VerifyRealMoneyPurchase?profileId=common_core&rvn=1') && !req.url.endsWith('Sentimos que tengas este error, estamos trabajando para impedir errores como este en el futuro si el error persiste contata con iKacs#4290 o HugoFastt#0001 mediante discord.')){
				console.log(req.url)
			}
			next()
		})
	...
	app.listen(process.env.PORT || 8080, () => {
		console.log(`SoftyFN Hibrido v${version} Conectado al puerto ${8080}!`);
	});
